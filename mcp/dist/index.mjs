...
var ROOT = process.env.FSM_DOCS_ROOT ? path.resolve(process.env.FSM_DOCS_ROOT) : path.resolve(path.dirname(new URL(import.meta.url).pathname), "..", "..");
var APPS = ["frappe", "erpnext"];
...
function doctypes() {
  if (!_doctypes) {
    _doctypes = APPS.flatMap(
      (app) => readJson(`index/doctypes.${app}.json`).doctypes
    );
  }
  return _doctypes;
}
var _whitelist = null;
function whitelist() {
  if (!_whitelist) _whitelist = readJson("index/whitelist.json").methods;
  return _whitelist;
}
var _hooks = null;
function hooks() {
  if (!_hooks) _hooks = readJson("index/hooks.json").hooks;
  return _hooks;
}
var _events = null;
function controllerEvents() {
  if (!_events) _events = readJson("index/controller-events.json");
  return _events;
}
var _pins = null;
function pins() {
...
var tagOf = (app) => pins().source[app].tag;
var devTagOf = (app) => pins().devcontainer[app].tag;
var pinNote = () => `production/CI: frappe ${tagOf("frappe")} \xB7 erpnext ${tagOf("erpnext")} (devcontainer runs ${devTagOf("frappe")} \u2014 see index/drift.md)`;
...
async function grepSource(pattern, opts = {}) {
...
function readSourceFile(app, rel, start, end) {
...
function parseFrontMatter(text2) {
  ...
  return { meta, body: text2.slice(end + 5) };
}
function pages() {
  if (_pages) return _pages;
  const found = [];
  for (const corpus of ["mirror", "vendor"]) {
...
var tokenize = (s) => s.toLowerCase().match(/[a-z0-9_]{2,}/g)?.filter((t) => !STOP.has(t)) ?? [];
var index = null;
function build() {
  const docs = [];
...
// src/index.ts
var APP_ENUM = { type: "string", enum: [...APPS], description: "Restrict to one app." };
var TOOLS = [
  {
    name: "find_doctype",
    description: "Search the ~810 DocTypes Frappe and ERPNext ship in production (frappe v16.31.0 / erpnext v16.32.1). Use this BEFORE designing a new DocType \u2014 the answer is often that the platform already has one. Returns each match with its module, required fields and flags, which is usually enough to tell whether it actually fits.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    ...
    description: "Full schema for one DocType as it exists in production: every field with its type and reqd/unique flags, permissions, links, child tables, autoname. Use this to answer 'does this actually fit our shape?' \u2014 a required field you cannot satisfy is the usual reason a close-sounding DocType is wrong.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "find_method",
    description: "Search the whitelisted (RPC-callable) method surface in production. Use before writing a new @frappe.whitelist endpoint, and to check whether an operation is already reachable from the client.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "find_hook",
    description: "Look up hooks.py keys the framework actually reads in production, and the document lifecycle event order (before_validate \u2192 validate \u2192 before_save \u2192 on_update \u2192 on_change, and the submit/cancel/delete/rename variants). If a hook key is absent here, nothing consumes it.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "grep_source",
    description: "Regex search the Frappe and ERPNext source at the production tag. This is the citation machine: use it to PROVE a platform claim before acting on it. Absence of something in our own repo is never evidence about the platform \u2014 this is.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "read_source",
    description: "Read a line range from a file in the production source tree. Use after grep_source to read the surrounding implementation before concluding what it does.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  {
    name: "search_docs",
    description: "Full-text search the mirrored docs.frappe.io prose plus the vendored frappe_docker and bench docs. Good for concepts and how-to. NOTE: upstream framework docs stop at v15 and ERPNext's are undated \u2014 for anything version-sensitive, confirm with grep_source at the pin rather than trusting the prose.",
    inputSchema: {
      type: "object",
      properties: {
...
function doctypeLine(d) {
  ...
  return [
    `${d.name}  [${d.app}/${d.module ?? "?"}${flags ? ` \xB7 ${flags}` : ""}]`,
    `    required: ${reqd}`,
    `    ${d.field_count} fields \xB7 ${d.json_path}`
  ].join("\n");
}
...
That is real evidence of absence \u2014 this index covers every DocType both apps ship. Consider grep_source for a non-DocType mechanism before building one.`
        );
      }
      const limit = a.limit ?? 25;
      const shown = hits.slice(0, limit);
      return text(
        `${hits.length} DocType(s) matching /${a.pattern}/i \xB7 ${pinNote()}
...
Use describe_doctype for the full field list before concluding one fits.`
    ...
    case "describe_doctype": {
      ...
      if (!d) {
        ...
        return text(
          `No DocType named "${a.name}" at ${pinNote()}` + (near.length ? `
...
Did you mean: ${near.slice(0, 8).map((n) => n.name).join(", ")}` : "")
      ...
      const lines = [
        `# ${d.name}`,
        `${d.app}/${d.module ?? "?"} \xB7 ${pinNote()}`,
        "",
        `- autoname: ${d.autoname ?? "(none)"}${d.naming_rule ? ` \xB7 naming_rule: ${d.naming_rule}` : ""}`,
        `- flags: submittable=${d.is_submittable} child_table=${d.istable} single=${d.issingle} tree=${d.is_tree} virtual=${d.is_virtual}`,
        `- track_changes=${d.track_changes} track_seen=${d.track_seen}${d.title_field ? ` \xB7 title_field=${d.title_field}` : ""}`,
        `- source: ${d.json_path}${d.controller_path ? ` \xB7 controller: ${d.controller_path}` : " \xB7 no controller"}`,
        "",
        `## Required fields (${d.reqd_fields.length})`,
        d.reqd_fields.length ? d.reqd_fields.map((f) => {
      ...
      if (mode === "all") {
        lines.push(
          ...
          "",
          `## Permissions`,
          d.permissions.length ? d.permissions.map((p) => `- ${p.role}: ${Object.keys(p).filter((k) => k !== "role").join(" ")}`).join("\n") : "- (none declared)"
        );
      }
    ...
    case "find_method": {
      ...
      if (hits.length === 0) return text(`No whitelisted method matching /${a.pattern}/i \xB7 ${pinNote()}`);
      const limit = a.limit ?? 30;
      return text(
        `${hits.length} whitelisted method(s) \xB7 ${pinNote()}
...
\u2026${hits.length - limit} more.` : "")
    ...
    case "find_hook": {
      if (a.lifecycle) {
        const ev = controllerEvents();
        const at = (spec, n) => spec.dispatched_at?.[n] ? `${n} (${spec.dispatched_at[n]})` : n;
        const rows = Object.entries(ev.actions).map(
          ([action, spec]) => `## ${action}
  before: ${spec.before.map((n) => at(spec, n)).join(" \u2192 ")}
  after:  ${spec.after.map((n) => at(spec, n)).join(" \u2192 ")}
  distinguished by ${spec.cite}`
        );
        return text(`# Document lifecycle \xB7 ${pinNote()}
...
${rows.join("\n\n")}`);
      ...
      if (hits.length === 0) {
        return text(
          `No hook key matching "${a.pattern}" \xB7 ${pinNote()}
...
This index is the set of keys the framework actually passes to get_hooks(), so an absence here means nothing would consume that hook.`
      ...
      return text(
        `${hits.length} hook key(s) \xB7 ${pinNote()}
...
` + hits.map((h) => `${h.hook}
    ...
    case "grep_source": {
      ...
      if (hits.length === 0) {
        return text(
          `No match for /${a.pattern}/ in ${a.app ?? "frappe + erpnext"} \xB7 ${pinNote()}

This searched the whole pinned tree, so it is evidence of absence in the platform \u2014 unlike a grep of our own repo, which only proves we have not used it.`
        );
      }
      return text(
        `${hits.length} match(es) \xB7 ${pinNote()}
...
` + hits.map((h) => `${h.app}/${h.file}:${h.line}
    ...
    case "read_source":
      return text(
        `${a.app}@${tagOf(a.app)} \xB7 ${a.path}

` + readSourceFile(a.app, a.path, a.start, a.end)
      );
    case "search_docs": {
      const prefix = a.scope && a.scope !== "all" ? SCOPE_PREFIX[a.scope] : void 0;
...
` + hits.map(
          (h) => `${h.title}  [${h.corpus}]${h.upstream_updated ? ` \xB7 upstream updated ${h.upstream_updated}` : ""}
    ${h.path}
` + (h.url ? `    ${h.url}
` : "") + `    ${h.snippet}`
        ).join("\n\n")
