---
title: "Configuration"
source_url: https://docs.frappe.io/framework/v15/user/en/basics/site_config
upstream_updated: "12-06-2026 22:19:24"
...
| `allow_cors` | Enables [cross-site HTTP requests](https://developer.mozilla.org/en-US/docs/Web/HTTP/CORS). Can be set to: - a specific origin, for example: `"https://example.com"` - a list of origins, for example: `["https://portal.example.com", "https://shop.example.com"]` - a wildcard: `"*"` (Not recommended for production use) |
...
| `monitor` | If set, logs all requests and saves under `./logs/monitor.json.log`. Check out [monitoring](https://docs.frappe.io/framework/v14/user/en/debugging#monitoring) under the Debugging documentation. |
| `mute_emails` | Stops email sending if true. |
| `pause_scheduler` | Pause Scheduler |
| `rate_limit` | Specify Rate Limits using `frappe.rate_limiter`. Check out the document reference [rate limiting](https://docs.frappe.io/framework/v14/user/en/rate-limiting). |
...
| `socketio_backend` | Specify Socket.IO Backend Server Implementaion. `node (default) or python` |
...
| `disable_transaction_log` | Disable creation of **Transaction Log** for **Sales Invoice** and **Payment Entry** (ERPNext Regional Feature). |
...
Guide to setup read operations from secondary MariaDB server in a replica setup is available at in the [user guides](https://docs.frappe.io/framework/v14/user/en/guides/database-settings/setup-read-from-secondary-db).
...
| `mail_login` | is used as the second preference and "[\[email protected\]](https://docs.frappe.io/cdn-cgi/l/email-protection#c7a9a8b3aea1aea4a6b3aea8a9b487a2bfa6aab7aba2e9a4a8aa)" is used as fallback. |
...
| `scheduler_tick_interval` | Job Scheduler runs a loop that enqueues all scheduled jobs pending execution once every _scheduler\_tick\_interval_ seconds. This makes it possible to set the minimum frequency for scheduling jobs. Defaults to 60. |
