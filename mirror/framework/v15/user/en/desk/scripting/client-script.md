---
title: "Client Script"
source_url: https://docs.frappe.io/framework/v15/user/en/desk/scripting/client-script
...
> The validations you add through Client Script will only be applied while using the standard form view accessible through the browser. In case you wish for those to be applied through API or [System Console](https://docs.frappe.io/framework/v14/user/en/desk/scripting/system-console) access too, use [Server Scripts](https://docs.frappe.io/framework/v14/user/en/desk/scripting/server-script).
...
The filters are referring to the **Contact** DocType and it's _Links_ child table.
...
`CommunicationComposer` (the email dialog) will check if there is a **Client Script** supplying default recipients. For example, if your **Quotation** DocType contains a field named _BCC_ (`"custom_bcc"`), you can add the following **Client Script** to use it as default BCC in the email dialog:
