---
title: "Item"
source_url: https://docs.frappe.io/erpnext/user/manual/en/item
upstream_updated: "12-03-2026 17:24:45"
...
-   **Valuation Rate**: There are two options to maintain the valuation of the stock. FIFO (first in - first out) and Moving Average. To understand this topic in detail, please visit [Item Valuation, FIFO and Moving Average](https://docs.frappe.io/erpnext/calculation-of-valuation-rate-in-fifo-and-moving-average).
-   **Standard Selling Rate**: When _creating_ an Item, entering a value for this field will automatically create an [Item Price](https://docs.frappe.io/erpnext/item-price) at the backend. Entering a value after the Item has been saved will not work. In this case, the Item Price is created from any transactions with the Item. The rate at which you'll sell the item. This will be fetched in Sales Orders and Sales Invoices.
-   **Is Fixed Asset**: Tick this checkbox if this item is a company Asset. Check out the [Asset Module](https://docs.frappe.io/erpnext/asset) to know more.
-   **Auto Create Assets on Purchase**: If Item is a Company Asset, tick this checkbox if you want to auto-create assets while purchasing this item through [Purchase Cycle](https://docs.frappe.io/erpnext/purchase-order). Check out the [Asset Page](https://docs.frappe.io/erpnext/asset) to know more.
-   **Allowance Percentage**: This option will be available only when you create and save the item. This is the percent by which you will be allowed to over-bill or over-deliver this Item. If not set, it will select from [Stock Settings](https://docs.frappe.io/erpnext/stock-settings#3-limit-percent).
...
![](https://docs.frappe.io/files/item_desc.png)

### 3.2 Barcodes

Barcodes can be recorded in Items to quickly scan and add them in transactions. In the Barcodes table, you can add an Item's [barcode for scanning](https://docs.frappe.io/erpnext/track-items-using-barcode). There are two types of barcodes in ERPNext:
...
-   **Shelf Life In Days**: This is for a product [Batch](https://docs.frappe.io/erpnext/batch). The number of days after which the product batch will be unusable. For example, medicines.
...
![](https://docs.frappe.io/files/item_inv.png)
...
-   **Valuation Method**: Select the Valuation Method, whether FIFO or Moving Average. Read [Item Valuation methods](https://docs.frappe.io/erpnext/calculation-of-valuation-rate-in-fifo-and-moving-average) to know more.
...
When the stock of an item dips under a certain quantity, you can set an automatic reorder under 'Auto Reorder' section. This should be enabled in [Stock Settings](https://docs.frappe.io/erpnext/stock-settings#9-automatic-material-request). This will raise a [Material Request](https://docs.frappe.io/erpnext/material-request) for the Item. The user with roles Purchase Manager and Stock Manager will be **notified** when the Material Request is created.
...
-   **Retain Sample**: To retain a minimum number of sample stock of the item. You need to set a Sample Retention Warehouse in Stock Settings for this. To know more, [click here](https://docs.frappe.io/erpnext/retain-sample-stock).
...
![](https://docs.frappe.io/files/item_bat_ser.png)

> **Note**: Once you mark an item as serialized or batched or neither, you cannot change it after you have made a Stock Entry.

To know more, visit the [Stock Reconciliation](https://docs.frappe.io/erpnext/stock-reconciliation) page.

### 3.8 Variants

An Item Variant is a different version of an Item. To learn more about managing variants, see [Item Variants](https://docs.frappe.io/erpnext/item-variants).
...
![](https://docs.frappe.io/files/item_def.png)
...
-   **Last Purchase Rate**: The rate at which you last purchased this item using a [Purchase Invoice](https://docs.frappe.io/erpnext/purchase-invoice) will be displayed here.
-   **Is Purchase Item:** If unticked, you won't be able to use this item in purchase transactions.
-   **Is Customer Provided Item:** Checked if Item is provided by a customer and received through **Stock Entry > Material Receipt**. If Checked, **Customer** field is Mandatory as the default customer for **Material Request**. To know more visit [this page](https://docs.frappe.io/erpnext/customer-provided-items).
...
-   **Delivered by Supplier (Drop Ship)**: If the item is delivered directly by the supplier to the customer, tick this checkbox. Read more [here](https://docs.frappe.io/erpnext/drop-shipping-in-erpnext).
-   **Supplier Codes:** Track Item Code defined by the Suppliers for this Item. In the Purchase transactions, on selecting an Item, a Supplier Part No. will be fetched as well for the Supplier's reference. You can read more about it [here](https://docs.frappe.io/erpnext/maintaining-suppliers-part-no-in-item).

![](https://docs.frappe.io/files/itmepur.png)

![](https://docs.frappe.io/files/item_supp.png)
...
-   **Grant Commission**: Grant a commission to [Sales Person](https://docs.frappe.io/erpnext/sales-person) and [Sales Partner](https://docs.frappe.io/erpnext/sales-partner) when this item is sold. If disabled, the sales generated by this item will be ignored in the calculation of commission.
...
![](https://docs.frappe.io/files/item_sales.png)
...
![](https://docs.frappe.io/files/item_defe.png)

Check out the pages on [Deferred Revenue](https://docs.frappe.io/erpnext/deferred-revenue) for more details.

### 3.15 Customer Details

The Customer may identify an Item with a different Item Code. this is Similar to [Supplier Code](https://docs.frappe.io/erpnext/item#311-supplier-details).
...
Read [Quality Inspection](https://docs.frappe.io/erpnext/quality-inspection) to know more.

### 3.18 Manufacturing

-   **Default BOM**: The default [Bill of Materials](https://docs.frappe.io/erpnext/bill-of-materials) used to manufacture this Item.
...
![](https://docs.frappe.io/files/item_man.png)
...
![](https://docs.frappe.io/files/Screenshot%202026-03-04%20at%204.23.37%E2%80%AFPM.png)
...
![](https://docs.frappe.io/files/Screenshot%202026-03-04%20at%204.23.45%E2%80%AFPM.png)

The following fields will be found in the Website Item master.

_Weightage_\*: Items with higher weight will be displayed first on the website. The limit for the number you can enter here is very high.

-   **Slideshow**: A slideshow can be displayed at the top of the page. Visit the [Homepage](https://docs.frappe.io/erpnext/homepage) page in the Website module to know more.
...
-   **Website Item Groups**: In this table, you can select existing or create new [Item Groups](https://docs.frappe.io/erpnext/item-group) to classify items on your website.
-   **Set Meta Tags**: Meta tags help with SEO. See [Web Page](https://docs.frappe.io/erpnext/web-page) to know how to add them.

Visit [Manufacturing](https://docs.frappe.io/erpnext/manufacturing) and [Website](https://docs.frappe.io/erpnext/website) to understand these topics in detail.
...
The hub is a free online marketplace where Suppliers and Customers can transact. If both parties are on ERPNext, the transactions happen seamlessly. You can visit the hub at: [https://hubmarket.org](https://hubmarket.org).

-   **Publish in Hub**: Choose if you want to publish your item on [https://hubmarket.org/](https://hubmarket.org/). It is a free marketplace. If your supplier/customer is also on ERPNext, the transactions will be seamless. For example, on creating a Purchase Order from your end, a Sales Order will be created on the Supplier's end.
...
1.  [Item Price](https://docs.frappe.io/erpnext/item-price)
2.  [Item Codification](https://docs.frappe.io/erpnext/item-codification)
3.  [Item Variants](https://docs.frappe.io/erpnext/item-variants)
4.  [Item Group](https://docs.frappe.io/erpnext/item-group)
5.  [Item Attribute](https://docs.frappe.io/erpnext/item-attribute)
6.  [Item Valuation FIFO And Moving Average](https://docs.frappe.io/erpnext/calculation-of-valuation-rate-in-fifo-and-moving-average)
