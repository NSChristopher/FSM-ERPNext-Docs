---
title: "Customer | ERPNext Documentation"
source_url: https://docs.frappe.io/erpnext/user/manual/en/customer
upstream_updated: "23-07-2026 16:46:30"
mirrored_from: docs.frappe.io (Frappe Wiki - not git-backed)
---

# Customer | ERPNext Documentation

A **Customer** represents a person, business, or other organization that buys goods or services from you. The Customer master gives quotations, orders, invoices, payments, projects, support records, and reports one consistent party reference.

Create a separate Customer for each independently billed party. Store people and locations as linked Contacts and Addresses instead of putting every detail in the Customer name.

![Customer details and sales defaults for Summit Digital Stores.](https://novacompanies.m.frappe.cloud/files/customer-details-and-defaults.png)

## Before you begin

Confirm the following before creating the record:

-   How Customer IDs should be generated in [Selling Settings](https://docs.frappe.io/erpnext/selling-settings). ERPNext can use the Customer name or a naming series.
-   The correct [Customer Group](https://docs.frappe.io/erpnext/customer-group), such as Commercial, Government, or a group designed for your reporting structure.
-   The Customer's [Territory](https://docs.frappe.io/erpnext/territory), billing currency, price list, payment terms, and expected credit policy.
...
-   **From a Lead:** Convert a qualified [Lead](https://docs.frappe.io/erpnext/lead) so the Customer remains connected to the earlier sales activity.
...
-   **Data import:** Use the [Data Import](https://docs.frappe.io/erpnext/data-import) tool for a reviewed batch of Customers. Import linked Contacts and Addresses separately and verify their Dynamic Link values.
...
| **Price List** | Selects the default selling [Price List](https://docs.frappe.io/erpnext/price-lists) used to retrieve Item prices. |
| **Payment Terms Template** | Applies the default [Payment Terms](https://docs.frappe.io/erpnext/payment-terms) schedule to new orders and invoices. |
...
[Addresses](https://docs.frappe.io/erpnext/address) and [Contacts](https://docs.frappe.io/erpnext/contact) are independent records linked to the Customer. This lets a Customer have several offices, delivery locations, buyers, and accounts-payable contacts without duplicating the Customer.
...
![Billing and shipping addresses with the primary purchasing contact.](https://novacompanies.m.frappe.cloud/files/customer-addresses-and-contacts.png)
...
-   Set a Tax Category or regional **Tax Withholding Category** when required by your tax configuration. See [Tax Withholding Category](https://docs.frappe.io/erpnext/tax-withholding-category) for the applicable rules.
-   Assign a [Sales Person](https://docs.frappe.io/erpnext/sales-person) or sales team and ensure contribution percentages total 100%.
-   Add a [Sales Partner](https://docs.frappe.io/erpnext/sales-partner) and commission rate when an external partner influences the sale.
-   Select a [Loyalty Program](https://docs.frappe.io/erpnext/loyalty-program) when the Customer earns or redeems loyalty points.
-   Use the same Customer across companies. Add a company-specific receivable account only when it is required by your [Chart of Accounts](https://docs.frappe.io/erpnext/chart-of-accounts).
...
![Company receivable account and Customer credit limit.](https://novacompanies.m.frappe.cloud/files/customer-accounting-and-credit-limit.png)

Use the dedicated [Credit Limit](https://docs.frappe.io/erpnext/credit-limit) guidance when defining the policy. The payment terms template controls when amounts are due; it does not increase the Customer's credit limit.
...
-   [Address](https://docs.frappe.io/erpnext/address)
-   [Quotation](https://docs.frappe.io/erpnext/quotation)
-   [Credit Limit](https://docs.frappe.io/erpnext/credit-limit)
