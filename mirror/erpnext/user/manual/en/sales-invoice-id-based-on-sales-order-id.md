...
source_url: https://docs.frappe.io/erpnext/user/manual/en/sales-invoice-id-based-on-sales-order-id
upstream_updated: "25-07-2026 12:34:36"
...
![ERPNext order-to-cash flow from Quotation through Payment Entry, with Sales Order highlighted as the current stage.](https://docs.frappe.io/files/order-to-cash-sales-order.webp)

The standard order-to-cash flow is **[Quotation](https://docs.frappe.io/erpnext/quotation) → Sales Order → [Delivery Note](https://docs.frappe.io/erpnext/delivery-note) → [Sales Invoice](https://docs.frappe.io/erpnext/sales-invoice) → [Payment Entry](https://docs.frappe.io/erpnext/payment-entry)**. You can also create a Sales Invoice directly from a submitted Sales Order when a separate delivery transaction is not required. You can also start without a Quotation or add picking, manufacturing, and purchasing steps.

## Before you begin

Create or confirm the following records:

-   a [Company](https://docs.frappe.io/erpnext/company) and [Customer](https://docs.frappe.io/erpnext/customer);
-   at least one [Item](https://docs.frappe.io/erpnext/item);
-   an [Item Price](https://docs.frappe.io/erpnext/item-price) or a rate you can enter manually;
-   a source [Warehouse](https://docs.frappe.io/erpnext/warehouse) if you deliver stock items;
-   [taxes](https://docs.frappe.io/erpnext/setting-up-taxes), [payment terms](https://docs.frappe.io/erpnext/payment-terms), and [shipping rules](https://docs.frappe.io/erpnext/shipping-rule) when your sales process requires them.
...
![A draft Sales Order for Northstar Retail showing two item rows, with the pencil icon highlighted for opening the row editor.](https://docs.frappe.io/files/sales-order-draft-items.webp)
...
For recurring requirements, use a Blanket Order or [Auto Repeat](https://docs.frappe.io/recurring-sales-orders-with-auto-repeat) only when the corresponding business process calls for it.
...
| Pricing | Price List | The [Price List](https://docs.frappe.io/erpnext/price-lists) from which ERPNext fetches item rates. Whether users can change a fetched Price List Rate is controlled in Selling Settings. |
| Pricing | Exchange Rate | Converts the transaction currency to company currency when they differ. |
| Pricing | Ignore Pricing Rule | Prevents configured [Pricing Rules](https://docs.frappe.io/erpnext/pricing-rule) from being applied. Use it only when the order requires an exception. |
...
| Items | Reserve Stock | Allocates available stock to the order when [stock reservation](https://docs.frappe.io/erpnext/stock-reservation) is configured; it does not create a stock-ledger movement. |
...
![The Create menu on a submitted Sales Order showing available fulfillment, billing, procurement, and production documents.](https://docs.frappe.io/files/sales-order-create-actions.webp)
...
For a **Maintenance** Sales Order, enable **Skip Delivery Note** when the service should be invoiced without a separate delivery transaction. See [Maintenance Sales Orders](https://docs.frappe.io/erpnext/maintenance-sales-orders) for the complete workflow.

  

![A full-screen Maintenance Sales Order showing the Skip Delivery Note checkbox beside Order Type.](https://docs.frappe.io/files/sales-order-skip-delivery-note-full.webp)
...
![The full Sales Order list showing draft, pending, and completed orders with delivery and billing progress.](https://docs.frappe.io/files/sales-order-list-statuses-full.webp)
...
-   **Maintenance** supports maintenance-related fulfillment. See [Maintenance Sales Orders](https://docs.frappe.io/erpnext/maintenance-sales-orders).
...
es. The order has a default Delivery Date, and individual item rows can use different dates.

### Can I deliver or invoice only part of an order?

Y

es. Create the Delivery Note or Sales Invoice for the required rows and quantities. ERPNext tracks the remaining percentages on the Sales Order.
...
-   [Partial Fulfilment of a Sales Order](https://docs.frappe.io/erpnext/partial-fulfilment-of-sales-order)
-   [Close Sales Order](https://docs.frappe.io/erpnext/close-sales-order)
-   [Amend a Sales Order after submission](https://docs.frappe.io/erpnext/amending-sales-order-after-submit)
