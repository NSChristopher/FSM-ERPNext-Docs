---
title: "Amending Sales Order after Submit | ERPNext Documentation"
source_url: https://docs.frappe.io/erpnext/user/manual/en/amending-sales-order-after-submit
upstream_updated: "24-07-2026 09:24:31"
mirrored_from: docs.frappe.io (Frappe Wiki - not git-backed)
---

# Amending Sales Order after Submit | ERPNext Documentation

After a [Sales Order](https://docs.frappe.io/erpnext/sales-order) is submitted, its core values are locked to protect the audit trail. ERPNext provides two ways to make a correction: use **Update Items** for permitted item-level changes, or cancel and amend the document when the change is broader.

## Before you begin

Check whether the Sales Order already has a submitted [Delivery Note](https://docs.frappe.io/erpnext/delivery-note), [Sales Invoice](https://docs.frappe.io/erpnext/sales-invoice), [Pick List](https://docs.frappe.io/erpnext/pick-list), or other linked transaction. Downstream documents determine which quantities and rates ERPNext can safely change.
...
> Do not cancel a submitted order only to reduce an outstanding balance that the customer no longer wants. [Close the Sales Order](https://docs.frappe.io/erpnext/close-sales-order) instead.
...
| Stop the remaining undelivered or unbilled balance | [Close](https://docs.frappe.io/erpnext/close-sales-order) |
| Fulfil or bill only part of the order now | [Partial Fulfilment](https://docs.frappe.io/erpnext/partial-fulfilment-of-sales-order) |
| Temporarily prevent further fulfilment | Hold |
| Reverse goods already delivered | [Sales Return](https://docs.frappe.io/erpnext/sales-return) |

## Troubleshooting

### Update Items is not visible

Confirm that the Sales Order is submitted, not cancelled or closed, and that your role has the required permissions. An administrator can review [Role-Based Permissions](https://docs.frappe.io/erpnext/role-based-permissions).
...
-   [Short Close Multiple Orders](https://docs.frappe.io/erpnext/short-close-multiple-orders)
